...
compare_xrefs_to_anchors <- function(path = ".", pattern = NULL, float = "figure") {
  get_wd <- getwd()
  setwd(path)
  ...
  first_figure_xref <- 
    figure_xrefs[, .(xref_page = min(xref_page)), by = "caption_id"]
  
  figure_captions <-
    aux_contents %>%
    .[grep("\\\\newlabel\\{fig[:][^@\\}]++\\}", AUX, perl = TRUE)] %>%
  ...
  figure_locations <- 
    aux_contents %>%
    .[grep("\\\\newlabel\\{fig[:][^@]++@cref\\}", AUX, perl = TRUE)] %>%
    .[, caption_id := sub("^\\\\newlabel\\{([^@]++)@.*+$", "\\1", AUX, perl = TRUE)] %>%
    .[, fig_no := sub("^.*\\[figure\\]\\[([12]?[0-9])\\]\\[([1-9])\\].*$", 
                      "\\2.\\1",
                      AUX,
                      perl = TRUE)] %>%
    .[, page := sub("^.*[^0-9]([0-9]+)\\}\\}$", "\\1", AUX, perl = TRUE)] %>%
  ...
  figure_locations[first_figure_xref, on = "caption_id", nomatch=0L] %>% 
    ...
    ggplot(aes(x = xref_page, y = fig_page, label = label)) + 
    geom_point() + 
