...
check_biber <- function(path = ".", .bib_warn_only = FALSE, rstudio = FALSE) {
    ...
    if (length(blg.file) == 1L){
      ...
      if (any(grepl("WARN ", blg, fixed = TRUE))){
        ...
        if (n_bad_entries > 1) {
          if (n_bad_entries > 5) {
            if(.bib_warn_only) {
            message("Biber emitted a warning. See the above WARN messages. (Only the first five are given; there were ",
            ...
            if(.bib_warn_only) {
            message("Biber emitted a warning. See the above WARN messages.")
            }
            if(!.bib_warn_only) {
